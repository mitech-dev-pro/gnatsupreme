...
export function logSlowRequests(request: Request, response: Response, next: NextFunction) {
  ...
  response.once("finish", () => {
    ...
    const staff = response.locals.user as
      { role?: string; regionId?: number | null; districtId?: number | null } | undefined;
    ...
    logger.warn(
      {
        ...
        scope: staff
          ? { role: staff.role, regionId: staff.regionId, districtId: staff.districtId }
          : member?.id
            ? { role: "MEMBER" }
            : undefined,
      },
