...
export const createBenefitPlanSchema = z
  .object({
    ...
    monthlyPremium: moneySchema.refine(
  ...
  .superRefine((value, context) => {
    for (const key of ["criticalIllness", "hospitalization"] as const) {
      if (value.benefits[key].spouseAmount !== null) {
        context.addIssue({
          code: "custom",
          path: ["benefits", key, "spouseAmount"],
          message: "Spouse cover is not available for this benefit",
        });
      }
    }
    if (
      value.benefits.criticalIllness.enabled &&
      value.benefits.criticalIllness.namedConditions.length === 0
    ) {
      context.addIssue({
        code: "custom",
        path: ["benefits", "criticalIllness", "namedConditions"],
        message: "Add at least one named critical illness",
      });
    }
    for (const key of ["death", "totalPermanentDisability", "hospitalization"] as const) {
      if (value.benefits[key].namedConditions.length > 0) {
        context.addIssue({
          code: "custom",
          path: ["benefits", key, "namedConditions"],
          message: "Named conditions are only available for Critical Illness",
        });
      }
    }
  });
