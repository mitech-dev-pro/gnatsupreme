...
export function validateClaimFile(file: {
  ...
  if (file.sizeBytes <= 0 || file.sizeBytes > CLAIM_MAX_FILE_BYTES)
    throw new ClaimDocumentError(
      `${file.originalName}: each file must be between 1 byte and 1 MB (1,000,000 bytes).`,
    );
  if (
    !CLAIM_MIME_EXTENSIONS[file.mimeType]?.includes(path.extname(file.originalName).toLowerCase())
  ) {
    throw new ClaimDocumentError(`${file.originalName}: use PDF, DOC, DOCX, JPEG or JPG.`);
  }
}
export async function loadClaimDocuments(
  claimType: string,
  documentIds: number[],
  memberId: number,
  ...
  if (files.length !== new Set(documentIds).size)
    throw new ClaimDocumentError("One or more claim documents are invalid or unavailable.");
  ...
  for (const file of files) {
    ...
    if (!field)
      throw new ClaimDocumentError(
        `${file.originalName}: this document category is not supported by Mankrado. Remove it or choose a supported document category.`,
      );
    if (fields.has(field))
      throw new ClaimDocumentError(
        `${file.originalName}: only one file is allowed per document category.`,
      );
    fields.add(field);
    validateClaimFile(file);
    const absolute = path.resolve(uploadRoot, file.storagePath);
    if (!absolute.startsWith(`${uploadRoot}${path.sep}`))
      throw new ClaimDocumentError("Invalid claim document path.");
    try {
      const info = await stat(absolute);
      validateClaimFile({ ...file, sizeBytes: info.size });
      if (!(await hasValidFileSignature(absolute, file.mimeType)))
        throw new ClaimDocumentError(`${file.originalName}: file content does not match its type.`);
  ...
  if (
    !hasRequiredDocuments(
      claimType,
      new Set(files.flatMap((file) => (file.slotKey ? [file.slotKey] : []))),
    )
  ) {
