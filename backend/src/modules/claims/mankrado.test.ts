...
import {
  deliverClaim,
  type ClaimDeliveryDependencies,
  type DeliveryClaim,
} from "./claims.delivery.js";

const input: ClaimSubmissionInput = {
  idempotencyKey: "staff:1:test",
  member: { id: 1, controllerId: "123456", fullName: "Test Member" },
  claimType: "HOSPITALIZATION",
  claimantType: "MEMBER",
  claimantName: "Test Member",
  claimantIdType: "GHANA_CARD",
  claimantIdNumber: "TEST",
...
const config = {
  enabled: true,
  mode: "API" as const,
  baseUrl: "https://example.test/gnatsupreme/",
  apiKey: "test-key",
};
...
test("all agreed file mappings are present and death_doc remains reserved", () => {
  assert.deepEqual(
    Object.values(CLAIM_FILE_FIELDS).flatMap(Object.values).sort(),
    [
      "death_med",
      "death_cert",
      "death_police",
...
test("file types and exact 1 MB boundary", () => {
  for (const [name, mime] of [
    ...
    assert.doesNotThrow(() =>
      validateClaimFile({ originalName: name!, mimeType: mime!, sizeBytes: 1_000_000 }),
    );
  }
  for (const sizeBytes of [0, 1_000_001])
    assert.throws(() =>
      validateClaimFile({ originalName: "a.pdf", mimeType: "application/pdf", sizeBytes }),
    );
  assert.throws(() =>
    validateClaimFile({ originalName: "a.png", mimeType: "image/png", sizeBytes: 10 }),
  );
  assert.throws(() =>
    validateClaimFile({ originalName: "a.exe", mimeType: "application/pdf", sizeBytes: 10 }),
  );
...
test("rejects missing/malformed status or claimId", () => {
  for (const body of [
    { status: "200" },
    { claimId: "MK-1" },
    { status: "500", claimId: "MK-1" },
    { status: "200", claimId: "" },
  ]) {
    assert.throws(
      () => parseSubmission(body),
      (error: unknown) => error instanceof ClaimsDeliveryError && error.outcome === "UNKNOWN",
    );
  }
});
...
test("HTTP failures, timeouts and malformed responses never imply acceptance", async () => {
  for (const [status, outcome] of [
    ...
    const provider = new MankradoClaimsProvider(
      config,
      async () => new Response("private provider error", { status }),
    );
    await assert.rejects(
      provider.createSubmission(input),
      (error: unknown) =>
  ...
  for (const transport of [
    ...
    await assert.rejects(
      new MankradoClaimsProvider(config, transport).createSubmission(input),
      (error: unknown) => error instanceof ClaimsDeliveryError && error.outcome === "UNKNOWN",
    );
  }
});
...
test("history and details use encoded path segments and normalize provider responses", async () => {
    ...
    return Response.json(
      urls.length === 1
        ? {
            ...
            history: [
              {
                claimNumber: 1788876358,
                name: "Test Member",
                claimDate: "2026-09-08",
                amountPayable: "20,000.0",
                claimStatus: "Awaiting Approval",
                staff_id: "123456",
              },
            ],
          }
        : {
            ...
            details: {
              claimNumber: "MC-1",
              name: "Test Member",
              claimDate: "2025-06-17 14:50:16.360",
  ...
  assert.deepEqual(urls, [
    "https://example.test/gnatsupreme/claims/history/123456",
    "https://example.test/gnatsupreme/claimdetails/MK%2F1",
  ]);
});

test("disabled configuration sends no requests", async () => {
  let calls = 0;
  const provider = new MankradoClaimsProvider({ ...config, enabled: false }, async () => {
    calls++;
    return Response.json({});
  });
...
function deliveryFixture(overrides: Partial<DeliveryClaim> = {}, fail = false) {
  const row: DeliveryClaim = {
    id: 1,
    ...
    claimantIdNumber: input.claimantIdNumber,
    claimantContact: { primaryPhone: "0200000000" },
    claimDetails: { hospitalName: "Test Hospital" },
  ...
  let sends = 0;
  const dependencies: ClaimDeliveryDependencies = {
    db: {
    ...
    provider: {
      ...
      createSubmission: async () => {
        sends++;
        if (fail) throw new ClaimsDeliveryError("UNKNOWN", "TIMEOUT", "Unconfirmed");
        return { externalClaimId: "MK-1", status: "SUBMITTED" };
      },
    },
...
test("member claims require review and historical simulations are never sent", async () => {
  for (const overrides of [
    ...
    { provider: "SIMULATION" },
  ] satisfies Partial<DeliveryClaim>[]) {
...
test("confirmed claim details envelope retains payment, rejection, date and display reference", () => {
  ...
  assert.equal(
    parseDetails(
      {
        ...payload,
        details: { ...payload.details, amountPayable: "0", rejectReason: "Document missing" },
      },
      "42",
    ).amountPayable,
    "0",
  ...
  assert.equal(
    parseDetails(
      {
        ...payload,
        details: { ...payload.details, amountPayable: null, name: null, claimDate: null },
      },
      "42",
    ).amountPayable,
    null,
  ...
  assert.throws(() =>
    parseDetails({ ...payload, details: { ...payload.details, amountPayable: "not money" } }, "42"),
  );
...
test("history parses confirmed envelope and comma-formatted amounts without discarding repeated rows", () => {
  ...
  assert.equal(
    parseHistory({
      status: 200,
      history: [{ ...item, claimNumber: "MC-1", amountPayable: "0" }],
    })[0]?.amountPayable,
    "0",
  );
  assert.equal(
    parseHistory({
      status: "200",
      history: [{ ...item, name: null, claimDate: null, amountPayable: null }],
    })[0]?.amountPayable,
    null,
  );
  for (const amountPayable of ["20,00.0", "abc", "-20", "1,000,00"])
    assert.throws(() => parseHistory({ status: "200", history: [{ ...item, amountPayable }] }));
  assert.throws(() => parseHistory({ status: "404", history: [] }));
  assert.throws(() =>
    parseHistory({ status: "200", history: [{ ...item, claimDate: "2026-02-31" }] }),
  );
});
