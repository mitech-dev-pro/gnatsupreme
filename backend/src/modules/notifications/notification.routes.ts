...
notificationRouter.get("/unread-count", async (_request, response) => {
  const count = await prisma.notification.count({
    where: { userId: user(response).id, channel: "IN_APP", readAt: null },
  });
...
notificationRouter.patch("/:id/read", async (request, response) => {
  ...
  const result = await prisma.notification.updateMany({
    where: { id: params.data.id, userId: user(response).id, channel: "IN_APP" },
    data: { readAt: new Date() },
  });
  if (!result.count) {
...
notificationRouter.post(
  ...
  async (request, response) => {
    ...
    const notification = await prisma.notification.findFirst({
      where: { id: params.data.id, channel: "SMS", status: "FAILED" },
    });
    if (!notification) {
