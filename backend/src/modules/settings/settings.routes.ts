...
async function applySettingsUpdate(
  ...
  if (existing.version !== expectedVersion)
    throw new SettingsConflict(
      `Settings were updated by another user. Current version is ${existing.version}.`,
    );
  const changedFields = Object.keys(values).filter(
    (key) => JSON.stringify(existing[key as keyof typeof existing]) !== JSON.stringify(values[key]),
  );
  if (!changedFields.length) return existing;
  const updated = await prisma.$transaction(async (transaction) => {
    const result = await transaction.organizationSettings.updateMany({
      where: { id: existing.id, version: expectedVersion },
      data: { ...values, version: { increment: 1 }, updatedById: actor.id },
    });
    if (result.count !== 1)
      throw new SettingsConflict("Settings changed while this request was being processed");
    const next = await transaction.organizationSettings.findUniqueOrThrow({
      where: { id: existing.id },
      include: { updatedBy: { select: { id: true, fullName: true, email: true } } },
    });
    await transaction.organizationSettingsVersion.create({
      data: {
        settingsId: next.id,
        version: next.version,
        snapshot: settingsSnapshot(next),
        changedFields,
  ...
  await recordAudit({
    ...
    entityId: updated.id,
    description: `Updated organization settings to version ${updated.version}`,
    beforeData: Object.fromEntries(
      changedFields.map((field) => [field, existing[field as keyof typeof existing]]),
    ),
...
settingsRouter.patch(
  ...
  async (request, response) => {
    ...
    try {
      response.json({
        success: true,
        data: await applySettingsUpdate(request, response, values, expectedVersion),
      });
    } catch (error) {
      if (error instanceof SettingsConflict) {
        response
          .status(409)
          .json({ success: false, message: error.message, data: await getOrganizationSettings() });
        return;
      }
      throw error;
    }
...
settingsRouter.post(
  ...
  async (request, response) => {
    ...
    try {
      response.json({
        success: true,
        data: await applySettingsUpdate(request, response, organizationDefaults, expectedVersion),
      });
    } catch (error) {
      if (error instanceof SettingsConflict) {
        response
...
settingsRouter.get(
  ...
  async (request, response) => {
    ...
    const [versions, total] = await prisma.$transaction([
      prisma.organizationSettingsVersion.findMany({
