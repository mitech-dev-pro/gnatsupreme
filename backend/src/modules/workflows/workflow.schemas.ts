...
export const createChangeRequestSchema = z.discriminatedUnion("type", [
  z.object({
    type: z.literal("MEMBER_DETAILS"),
    proposedData: memberDetailsChangeSchema,
    requestNote: z.string().trim().max(500).optional(),
  }),
  z.object({
    type: z.literal("SPOUSE"),
    proposedData: spouseSchema,
    requestNote: z.string().trim().max(500).optional(),
  }),
  z.object({
    type: z.literal("BENEFICIARY_ADD"),
    proposedData: beneficiarySchema,
    requestNote: z.string().trim().max(500).optional(),
  }),
  z.object({
    type: z.literal("BENEFICIARY_UPDATE"),
    targetBeneficiaryId: z.coerce.number().int().positive(),
    proposedData: beneficiarySchema,
    requestNote: z.string().trim().max(500).optional(),
  }),
  z.object({
    type: z.literal("BENEFICIARY_REMOVE"),
    targetBeneficiaryId: z.coerce.number().int().positive(),
    requestNote: z.string().trim().max(500).optional(),
  }),
]);
