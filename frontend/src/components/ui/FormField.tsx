...
function Frame({
  ...
  return (
    <div>
      <label htmlFor={id} className="mb-1.5 block text-xs font-bold text-text-strong">
        {label}
        {required && (
      ...
      {(hint || error) && (
        <p
          ...
          className={`mt-1.5 text-xs ${error ? "font-semibold text-danger" : "text-text-muted"}`}
        >
          {error ?? hint}
        </p>
      )}
  ...
  return (
    <Frame id={id} label={label} hint={hint} error={error} required={required}>
      <textarea
        ...
        aria-invalid={Boolean(error) || undefined}
        aria-describedby={error ? `${id}-error` : hint ? `${id}-hint` : undefined}
        className={`${controlClasses} min-h-24 resize-y ${error ? "border-danger" : ""} ${className}`}
        {...props}
      />
    </Frame>
  );
