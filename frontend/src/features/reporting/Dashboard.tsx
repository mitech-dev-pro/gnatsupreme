    ...
    <div className="dash-bars">
      {data.map((item) => (
        <div className="dash-bar" key={item.month}>
          ...
          <span style={{ height: `${Math.max(5, (item.count / max) * 128)}px` }} />
          <small>
            {new Date(`${item.month}-01T00:00:00`).toLocaleDateString(undefined, {
              month: "short",
            })}
          </small>
        </div>
    ...
    <div className="dashboard">
      <header className="dash-heading">
        <div>
          ...
          <span>A quick view of membership operations and current exceptions.</span>
        </div>
      ...
      <section className="dash-metrics" aria-label="Membership summary">
        ...
        <Metric
          ...
          value={data.members.flagged + data.members.returned + data.members.inactive}
          icon={icons.flagged}
          tone="red"
        />
      </section>
        ...
        <section className="dash-panel dash-quick">
          ...
          <dl>
            <div>
              ...
              <dd>{(data.coverage.spouses + data.coverage.beneficiaries).toLocaleString()}</dd>
            </div>
            <div>
          ...
          {claimsTotal === 0 && (
            <p className="dash-quick__note">
              Claim status totals will appear here after Mankrado begins sending submissions.
            </p>
          )}
        </section>
      </div>
