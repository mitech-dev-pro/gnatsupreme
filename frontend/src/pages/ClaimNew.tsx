...
export default function ClaimNew() {
  ...
  const policyPageComplete = Boolean(
    claimType && incidentDate && claimantIdNumber.trim(),
  );
  const missingPolicyItems = [
    !claimType && "claim type",
    !incidentDate && "incident date",
    !claimantIdNumber.trim() && "Ghana Card number",
  ...
  return <div className="mx-auto max-w-[960px] pb-6">
    ...
    <main className="overflow-hidden rounded-[12px] border border-(--border-default) bg-(--surface-raised) shadow-[0_2px_6px_rgba(30,39,97,0.08)]">
      ...
      <div className="p-5 sm:p-7">
        ...
        {page === 0 && <form onSubmit={(event) => { event.preventDefault(); if (policyPageComplete) { setError(""); setPage(1); } }}>
          ...
          <div className="mt-7"><SectionTitle>Mode of identification</SectionTitle><div className="grid gap-4 sm:grid-cols-2"><ReadOnlyField label="Claimant ID type" value="Ghana Card"/><InputField label="Ghana Card number" required value={claimantIdNumber} onChange={(event) => setClaimantIdNumber(event.target.value)} placeholder="GHA-000000000-0"/></div></div>
          ...
          <div className="mt-7 rounded-[9px] bg-(--surface-subtle) p-4"><div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button variant="secondary" onClick={() => { setMember(null); setPage(-1); }}>Back</Button><Button type="submit" disabled={!policyPageComplete}>Next</Button></div>{!policyPageComplete && <p className="mt-3 text-right text-[11.5px] font-semibold text-(--text-muted)" role="status">Complete: {missingPolicyItems.join(", ")}.</p>}</div><Progress page={page}/>
        </form>}
