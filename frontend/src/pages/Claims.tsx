...
export default function Claims() {
  ...
  const load = useCallback(async () => { setLoading(true); setError(""); try { const response = await api.get("/claims/submissions", { params: { page, limit, status: status || undefined } }); setRows(response.data.data); setTotal(response.data.pagination.total); setTotalPages(Math.max(1, response.data.pagination.totalPages)); } catch { setError("Claim submissions could not be loaded."); } finally { setLoading(false); } }, [page, status]);
