...
export default function Report20Upload() {
  ...
  return (
    <div>
      ...
      <h1 className="mb-1 text-[22px] font-extrabold text-[#1e2761]">
        Report 20 Reconciliation
      </h1>
      <div className="mb-5 text-[12.5px] text-[#5b6472]">
        Upload the monthly Report 20 payroll deductions file to reconcile it
        against enrolled members. This is a separate file format from bulk
        member enrollment — it has payroll fields (Employee No, Amount,
        Balance, District, Reference), not member/beneficiary details.
      </div>
      ...
      >
        ...
        >
          ...
          <div className="mt-1 text-[11.5px] text-[#5b6472]">CSV or XLSX</div>
          <input
            ref={fileInputRef}
            type="file"
              ...
              {jobsLoading ? (
                ...
                jobs.map((job) => (
                  <tr key={job.id} className="border-b border-[#e5e9f0] last:border-0 hover:bg-[#fafbfd]">
                    ...
                    <td className="px-4 py-2.5 text-[#17805f]">{job.matchedRows}</td>
                    <td className="px-4 py-2.5 text-[#c23b3b]">{job.unmatchedRows}</td>
                    <td className="px-4 py-2.5">
                      <span
